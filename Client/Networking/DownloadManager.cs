...
namespace RDRNetwork.Networking
{
    internal static class DownloadManager
    {
        ...
        {
            foreach (var asiMod in Util.Util.GetModules().Where(mod => mod.ModuleName.EndsWith(".asi")))
            {
        ...
        internal static void DownloadPart(int id, byte[] bytes)
        {
            ...
            {
                LogManager.DebugLog("Downloading " +
        ...
        internal static void End(int id)
        {
            if (CurrentFile == null || CurrentFile.Id != id)
            {
                ...
                LogManager.DebugLog($"END Channel mismatch! We have {CurrentFile?.Id} and supplied was {id}");
                return;
            }
            ...
            try
            {
                if (CurrentFile.Type == FileType.Map)
                {
                    ...
                    if (obj == null)
                    {
                       LogManager.DebugLog("ERROR DOWNLOADING MAP: NULL");
                    }
                    else
                    {
                        Main.AddMap(obj);
            ...
            finally
            {
                ...
                if (CurrentFile.Type == FileType.Normal && File.Exists(CurrentFile.FilePath))
                {
                    ...
                    if (!_allowedFiletypes.Contains(mime))
                    {
                        ...
                        LogManager.DebugLog("Disallowed file type: " + mime + "~n~" + CurrentFile.Filename);
                    }
                }
    ...
    {
        internal static string _DOWNLOADFOLDER_ = Main.RDRNetworkFolder + "\\resources\\";
