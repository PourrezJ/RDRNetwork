...
System::UInt64 EncodeObject(System::Object^ obj)
{
	...
	if (type == RDR2::Native::InputArgument::typeid) {
		return static_cast<RDR2::Native::InputArgument^>(obj)->_data;

	} else if (type == RDR2::Native::OutputArgument::typeid) {
		return System::IntPtr(static_cast<RDR2::Native::OutputArgument^>(obj)->GetPointer()).ToInt64();

	} else if (RDR2::Native::INativeValue::typeid->IsAssignableFrom(type)) {
		return static_cast<RDR2::Native::INativeValue^>(obj)->NativeValue;
		...
		return RDR2::Native::Function::AddStringPool(static_cast<System::String^>(obj)).ToInt64();
	...
	RDR2::WriteLog("*** Tried encoding unhandled type {0}:\n{1}", type->FullName, System::Environment::StackTrace);
	return 0;
}
...
System::Object^ DecodeObject(System::Type^ type, System::UInt64* value)
{
	...
	} else if (type == RDR2::Math::Vector2::typeid) {
		const auto data = reinterpret_cast<const float*>(value);
		return gcnew RDR2::Math::Vector2(data[0], data[2]);

	} else if (type == RDR2::Math::Vector3::typeid) {
		const auto data = reinterpret_cast<const float*>(value);
		return gcnew RDR2::Math::Vector3(data[0], data[2], data[4]);

	} else if (RDR2::Native::INativeValue::typeid->IsAssignableFrom(type)) {
		// Warning: Requires classes implementing 'INativeValue' to repeat all constructor work in the setter of 'NativeValue'
		auto ret = static_cast<RDR2::Native::INativeValue^>(System::Runtime::Serialization::FormatterServices::GetUninitializedObject(type));
