...
public:
	...
	{
		RDR2::WriteLog("*** Unhandled exception: {0}", args->ExceptionObject->ToString());
	}
};
...
void LoadScriptDomain()
{
	...
	auto cefpath = System::IO::Path::GetFullPath(curDir + "..\\..\\cef");

	RDR2::WriteLog("Creating AppDomain with base \"{0}\"", setup->ApplicationBase);
	...
	RDR2::ManagedGlobals::g_appDomain = System::AppDomain::CreateDomain(appDomainName, nullptr, setup, appDomainPermissions);
	RDR2::ManagedGlobals::g_appDomain->InitializeLifetimeService();

	RDR2::WriteLog("Created AppDomain \"{0}\"", RDR2::ManagedGlobals::g_appDomain->FriendlyName);

	

	auto typeScriptDomain = RDR2::ScriptDomain::typeid;
	try {
		RDR2::ManagedGlobals::g_scriptDomain = static_cast<RDR2::ScriptDomain^>(RDR2::ManagedGlobals::g_appDomain->CreateInstanceFromAndUnwrap(typeScriptDomain->Assembly->Location, typeScriptDomain->FullName));
	}
	catch (System::Exception ^ ex) {
		RDR2::WriteLog("*** Failed to create ScriptDomain: {0}", ex->ToString());
		if (ex->InnerException != nullptr) {
			RDR2::WriteLog("*** InnerException: {0}", ex->InnerException->ToString());
		}
		return;
	}
	catch (...) {
		RDR2::WriteLog("*** Failed to create ScriptDomain beacuse of unmanaged exception");
		return;
	}

	RDR2::WriteLog("Created ScriptDomain!");

	RDR2::ManagedGlobals::g_scriptDomain->FindAllTypes();
}

static bool ManagedScriptInit(int scriptIndex, void* fiberMain, void* fiberScript)
{
	return RDR2::ManagedGlobals::g_scriptDomain->ScriptInit(scriptIndex, fiberMain, fiberScript);
}
...
static bool ManagedScriptExists(int scriptIndex)
{
	...
	return RDR2::ManagedGlobals::g_scriptDomain->ScriptExists(scriptIndex);
}

static int ManagedScriptGetWaitTime(int scriptIndex)
{
	return RDR2::ManagedGlobals::g_scriptDomain->ScriptGetWaitTime(scriptIndex);
}

static void ManagedScriptResetWaitTime(int scriptIndex)
{
	RDR2::ManagedGlobals::g_scriptDomain->ScriptResetWaitTime(scriptIndex);
}

static void ManagedScriptTick(int scriptIndex)
{
	RDR2::ManagedGlobals::g_scriptDomain->ScriptTick(scriptIndex);
}
...
{
	RDR2::WriteLog("SHVDN Pro Initializing");
...
static void ManagedSHVDNProControl()
{
	void* fiber = CreateFiber(0, [](void*) {
		RDR2::WriteLog("Control thread initializing");
...
static void StopRDRN() {
	if (started) {
		...
		UnmanagedLogWrite("RDRNetwork stopped...\n");
	}
	else {
		started = true;
		// start all parts
		SHVDNProControl();
		UnmanagedLogWrite("RDRNetwork is back!\n");
	}
}
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD fdwReason, LPVOID lpvReserved)
{
	if (fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		UnmanagedLogWrite("RDRNetwork Module loading ... please wait!\n");
		...
		UnmanagedLogWrite("Hook Initialization...\n");
		auto status = MH_Initialize();
		UnmanagedLogWrite("Minhook init_status : %s\n", std::string(MH_StatusToString(status)));


		_instance = hModule;
		SetEnvironmentVariableA("SHVDNPro", "RDRNetwork");
		...
		ManagedInitialize();
		scriptRegister(hModule, SHVDNProControl);
		for (int i = 0; i < 21; i++) {
