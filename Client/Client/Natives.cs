...
namespace RDRNetwork
{
    ...
    {
        private static List<int> EntityCleanup;
        private static List<int> BlipCleanup;
        ...
        internal static IEnumerable<object> DecodeArgumentList(params NativeArgument[] args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                {
                    list.Add(Streamer.NetToEntity(((EntityArgument)arg).NetHandle)?.Handle);
                }
                else if (arg is EntityPointerArgument)
                {
                    list.Add(new OutputArgument(Streamer.NetToEntity(((EntityPointerArgument)arg).NetHandle)));
                }
                else if (args == null)
                {
                    list.Add(null);
        ...
        internal static IEnumerable<object> DecodeArgumentListPure(params NativeArgument[] args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                {
                    var tmp = (Vector3Argument)arg;
                    list.Add(new RDRNetworkShared.Vector3(tmp.X, tmp.Y, tmp.Z));
                }
                else if (arg is LocalGamePlayerArgument)
                {
                ...
                {
                    var ent = Streamer.NetToStreamedItem(((EntityArgument)arg).NetHandle);
                    ...
                    {
                        list.Add(new LocalHandle(Streamer.NetToEntity(ent)?.Handle ?? 0));
                    }
                    else
                    {
        ...
        internal static List<NativeArgument> ParseNativeArguments(params object[] args)
        {
            ...
            foreach (var o in args)
            {
                ...
                else if (o is RDRNetworkShared.Vector3)
                {
                    var tmp = (RDRNetworkShared.Vector3)o;
                ...
                {
                    list.Add(new EntityArgument(Streamer.EntityToNet(((LocalHandle)o).Value)));
                }
                else if (o is IList)
                {
        ...
        internal static void DecodeNativeCall(NativeData obj)
        {/*
            ...
            */
            ...
            if (((int)nativeType & (int)NativeType.VehicleWarp) > 0)
            {
                ...
                var item = Streamer.NetToStreamedItem(veh);
                if (item != null && !item.StreamedIn) Streamer.StreamIn(item);
            }
            ...
            if (((int)nativeType & (int)NativeType.TimeSet) > 0)
            {
                ...
                Main.Time = new TimeSpan(newHours, newMinutes, 0);
            }
            ...
            if (((int)nativeType & (int)NativeType.WeatherSet) > 0)
            {
                ...
                {
                    Main.Weather = _weather[newWeather];
        ...
        internal static void SendNativeCallResponse(uint id, object response)
        {
            ...
            var bin = Main.SerializeBinary(obj);
            msg.Write((byte)PacketType.NativeResponse);
            msg.Write(bin.Length);
            msg.Write(bin);
            Main.Client.SendMessage(msg, NetDeliveryMethod.ReliableOrdered, 0);
        }

        private static bool ReplacePointerNatives(ulong hash, ref List<InputArgument> list, ref List<NativeArgument> args)
        {
        ...
        private static NativeType CheckNativeHash(ulong hash)
        {
            switch (hash)
            {
                default:
