...
using System.Linq;
using Vector3 = RDR2.Math.Vector3;
using WeaponHash = RDR2.WeaponHash;
using VehicleHash = RDR2.Native.VehicleHash;
using RDR2;
...
namespace RDRNetwork.Sync
{
    ...
    internal partial class SyncPed : RemotePlayer
    {
        ...
        internal int VehicleHash
        {
            get
            {
                if (!Debug)
                {
                    ...
                    var car = Streamer.NetToStreamedItem(VehicleNetHandle) as RemoteVehicle;
        ...
        internal long TicksSinceLastUpdate => (DateTime.Now.Ticks / 10000) - LastUpdateReceived;
        ...
        bool CreateCharacter()
        {
            ...
            if ((Character == null || !Character.Exists()) || (Character.Model.Hash != ModelHash || (Character.IsDead && PedHealth > 0)))
            {
                ...
                Character = World.CreatePed(charModel, gPos, _rotation.Z, false);
                charModel.MarkAsNoLongerNeeded();

                if (Character == null) return true;

                lock (Streamer.ClientMap) Streamer.HandleMap.Set(RemoteHandle, Character.Handle);
...
/*
                ...
                */
                Streamer.ReattachAllEntities(this, false);

                foreach (var source in Streamer.ClientMap.Values.Where(item => item is RemoteParticle && ((RemoteParticle)item).EntityAttached == RemoteHandle).Cast<RemoteParticle>())
                {
                    Streamer.StreamOut(source);
                    Streamer.StreamIn(source);
                }

                if (PacketOptimization.CheckBit(Flag, EntityFlag.Collisionless))
                {
                    Character.SetNoCollision(Character, false);
                }

               // JavascriptHook.InvokeStreamInEvent(new LocalHandle(Character.Handle), (int)RDRNetworkShared.EntityType.Player);
        ...
        void DrawNametag()
        {
            ...
            if (((Character.IsInRangeOfEx(Game.Player.Character.Position, 25f))) || Function.Call<bool>(Hash.IS_PLAYER_FREE_AIMING_AT_ENTITY, Game.Player, Character)) //Natives can slow down
            {
                if (Function.Call<bool>(Hash.HAS_ENTITY_CLEAR_LOS_TO_ENTITY, Game.Player.Character, Character, 17)) //Natives can slow down
                {
                    ...
                    Vector3 targetPos;
                    targetPos = Character.GetBoneCoord(Bone.SKEL_HEAD) + new Vector3(0, 0, 0.5f);
                    ...
                    Util.Util.DrawText(nameText.Replace("_", " "), 0, 0, 0.53f * sizeOffset, defaultColor.R, defaultColor.G, defaultColor.B, 255, 4, 1, false, true, 0);
        ...
        bool CreateVehicle()
        {
            ...
            if (_isInVehicle && MainVehicle != null && Character.IsInVehicle(MainVehicle) && PlayerChar.IsInVehicle(MainVehicle) && VehicleSeat == -1 && Function.Call<int>(Hash.GET_SEAT_PED_IS_TRYING_TO_ENTER, PlayerChar) == -1 && PlayerChar.GetPedSeat() == 0)
            {
                ...
                PlayerChar.Task.WarpIntoVehicle(MainVehicle, -1);
            ...
            {
                createVehicle = createVehicle || Streamer.EntityToNet(MainVehicle.Handle) != VehicleNetHandle;
            }
            if (!createVehicle) return false;

            MainVehicle = new Vehicle(Streamer.NetToEntity(VehicleNetHandle)?.Handle ?? 0);
            ...
            /* Besoin de récup le siège actuel
        ...
        private void VMultiVehiclePos()
        {
            ...
            {
                MainVehicle.Quaternion = RDR2.Math.Quaternion.Slerp(_lastVehicleRotation.Value.ToQuaternion(),
        ...
        void DisplayVehiclePosition()
        {
            ...
            {
                var dir = Position - _lastPosition.Value;
                var posTarget = Util.LinearVectorLerp(_carPosOnUpdate, Position + dir, (int)DateTime.Now.Subtract(_stopTime).TotalMilliseconds, 1000);
            ...
            {
                MainVehicle.Quaternion = RDR2.Math.Quaternion.Slerp(_lastVehicleRotation.Value.ToQuaternion(), _vehicleRotation.ToQuaternion(), Math.Min(1.5f, TicksSinceLastUpdate / (float)AverageLatency));
            }
            else
            {
                MainVehicle.Quaternion = _vehicleRotation.ToQuaternion();
        ...
        bool DisplayVehicleDriveBy()
        {
            ...
            var end = start + VehicleRotation.RotationToDirection() * 100f;
            ...
            else if ((VehicleHash)VehicleHash == GTA.Native.VehicleHash.Savage || (VehicleHash)VehicleHash == GTA.Native.VehicleHash.Hydra || (VehicleHash)VehicleHash == GTA.Native.VehicleHash.Lazer)
            {
        ...
        void UpdateVehicleMountedWeapon()
        {
            if (WeaponDataProvider.DoesVehicleSeatHaveGunPosition((VehicleHash)VehicleHash, VehicleSeat))
            {
                ...
                if (IsShooting)
                {/*
                    if (((VehicleHash)VehicleHash == RDR2.Native.VehicleHash.Rhino &&
                         DateTime.Now.Subtract(_lastRocketshot).TotalMilliseconds > 1000) ||
                        ((VehicleHash)VehicleHash != RDR2.Native.VehicleHash.Rhino))
                    {
                        _lastRocketshot = DateTime.Now;
                        ...
                        var damage = WeaponDataProvider.GetWeaponDamage(WeaponHash.ShotgunSawedoff);
            ...
            else if (!WeaponDataProvider.DoesVehicleSeatHaveMountedGuns((VehicleHash)VehicleHash) || VehicleSeat != -1)
            {
                if (Character.Weapons.Current.Hash != (WeaponHash)CurrentWeapon)
                {
                    ...
                    Character.Weapons.Give((uint)CurrentWeapon, -1, true, true);
                }
                ...
                if (IsShooting || IsAiming)
                {
                    ...
                    if (IsShooting)
                    {
                        Function.Call(Hash.SET_PED_INFINITE_AMMO, Character, true);
                    ...
                    if (Function.Call<bool>(Hash.HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY, Game.Player.Character, Character, true))
                    {
                        ...
                        JavascriptHook.InvokeCustomEvent(api =>
                            api.invokeonLocalPlayerDamaged(them, CurrentWeapon, boneHit));*/
                    }
        ...
        void DisplayParachuteFreefall()
        {
            ...
            Character.Quaternion = RDR2.Math.Quaternion.Slerp(Character.Rotation.ToQuaternion(), _rotation.ToQuaternion(),
        ...
        void DisplayMeleeAnimation()
        {
            ...
            if (!meleeSwingDone && CurrentWeapon != unchecked((int)WeaponHash.Unarmed))
            {
                ...
                if (gunEntity != null)
                {
                    ...
                    var ray = World.Raycast(start, end, (int)Math.Abs(end.X - start.X),
...
*/
                        if (!Streamer.LocalCharacter.IsInvincible)
            ...
            {
                var rightfist = Character.GetBoneCoord(Bone.SKEL_R_HAND);
...
*/
        ...
        bool UpdateOnFootPosition()
        {
            ...
            if (EnteringVehicle && !_lastEnteringVehicle)
            {
                ...
                {
                    targetVeh = Streamer.NetToEntity(VehicleNetHandle);
                }
                ...
                if (targetVeh != null)
                {
                    ...
                    Character.Task.EnterVehicle(new Vehicle(targetVeh.Handle), (RDR2.VehicleSeat)VehicleSeat, -1, 2f);
        ...
        internal void VMultiOnfootPosition()
        {
            ...
            */
            ...
            if (length > 0.05f * 0.05f && length < StreamerThread.PlayerStreamingRange * StreamerThread.PlayerStreamingRange)
            {
                ...
                {
                    Character.PositionNoOffset = RDR2.Math.Vector3.Lerp(
                        new RDR2.Math.Vector3(Character.Position.X, Character.Position.Y, Character.Position.Z),
                        new RDR2.Math.Vector3(Position.X, Position.Y, Position.Z), lerpValue);
                }
                else
                {
                    var tmpPosition = Vector3.Lerp(
                        new Vector3(Character.Position.X, Character.Position.Y, Character.Position.Z),
                        new RDR2.Math.Vector3(
            ...
            {
                var posTarget = Util.LinearVectorLerp(_carPosOnUpdate, Position + (Position - (_lastPosition ?? Position)),
            ...
            //Character.Quaternion = RDR2.Math.Quaternion.Lerp(Character.Quaternion, Rotation.ToQuaternion(), 0.10f); // mise à jours de la rotation
            Character.Quaternion = RDR2.Math.Quaternion.Lerp(Character.Rotation.ToQuaternion(), Rotation.ToQuaternion(), 0.10f); // mise à jours de la rotation
                    ...
                    if (IsAiming && !IsReloading)
                    {
                        ...
                        if (length < (StreamerThread.PlayerStreamingRange * StreamerThread.PlayerStreamingRange) / 2)
                        {
                            ...
                            else if ((!isAiming) && OnFootSpeed > 0)
                            {
                                ...
                                Function.Call(Hash.TASK_GO_TO_ENTITY_WHILE_AIMING_AT_ENTITY, Character, _followProp, _aimingProp, (float)OnFootSpeed, false, 10000, 10000, true, true, (uint)FiringPattern.Default);
        ...
        internal static Ped GetResponsiblePed(Vehicle veh)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (veh.GetPedOnSeat(i).Handle != 0) 
                    return veh.GetPedOnSeat(i);
            }
        ...
        private void UpdatePlayerPedPos(bool updateRotation = true, bool fixWarp = true)
        {
            ...
            if (updateRotation)
            {
                ...
                {
                    /*
                    Character.Quaternion = Quaternion.Slerp(Character.Quaternion, _rotation.ToQuaternion(),
                        Math.Min(1f, (DataLatency + TicksSinceLastUpdate) / (float)AverageLatency));*/
                }
                else
                {
        ...
        internal int GetWeaponHandsHeld(int weapon)
        {
            ...
            if (weapon == unchecked((int)WeaponHash.GolfClub) ||
                weapon == unchecked((int)ResurrectionMPShared.WeaponHash.Poolcue) ||
            ...
            if (weapon == unchecked((int)WeaponHash.Knife) || weapon == unchecked((int)WeaponHash.Nightstick) ||
                ...
                weapon == unchecked((int)ResurrectionMPShared.WeaponHash.Wrench) ||
                weapon == unchecked((int)ResurrectionMPShared.WeaponHash.Battleaxe) ||
            ...
            if (weapon == -1357824103 || weapon == -1074790547 ||
                ...
                weapon == unchecked((int)ResurrectionMPShared.WeaponHash.Autoshotgun) ||
        ...
        internal void Clear()
        {
            ...
            lock (Streamer.ClientMap) Streamer.HandleMap.Remove(RemoteHandle);
        }
